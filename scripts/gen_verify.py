#!/usr/bin/env python3
"""Generate verify.sh script to reproduce maintenance downtime deduction flow."""
...
resp=$(curl -s -X POST "$API/lease-orders" \
  -H "Content-Type: application/json" \
  -d "{
...
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/running-hours" \
  -H "Content-Type: application/json" \
  -d '{"reportDate": "2025-01-15", "reportedHours": 8}')
...
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/running-hours" \
  -H "Content-Type: application/json" \
  -d '{"reportDate": "2025-01-16", "reportedHours": 12}')
assert_contains "上报成功" '"code":0\|"code":200' "$resp"

# ----------------------------- f) 登记维修 1月15日 08:00-12:00 停机4小时
echo
blue "[f] 登记维修：2025-01-15 08:00~12:00，停机 4 小时，费用 500 元"
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/maintenance" \
  -H "Content-Type: application/json" \
  -d '{
    "startTime": "2025-01-15T08:00:00+08:00",
    "endTime":   "2025-01-15T12:00:00+08:00",
...
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/settlement/calculate" \
  -H "Content-Type: application/json")
...
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/settlement/review" \
  -H "Content-Type: application/json")
...
resp=$(curl -s -X POST "$API/lease-orders/$ORDER_ID/settlement/confirm" \
  -H "Content-Type: application/json")
...
resp=$(curl -s -w "\\nHTTP_CODE:%{http_code}\\n" -X PUT \
  "$API/lease-orders/$ORDER_ID/maintenance/$MT1_ID" \
  -H "Content-Type: application/json" \
  -d '{
...
if [ "$HTTP_CODE" = "200" ] && echo "$resp" | grep -q '"code":0\|"code":200'; then
  ...
  green "  ✓ PASS: 锁定后修改维修记录被正确拒绝 (HTTP=$HTTP_CODE)"
  pass=$((pass+1))
fi
...
if [ "$HTTP_CODE" = "200" ] && echo "$resp" | grep -q '"code":0\|"code":200'; then
  ...
  green "  ✓ PASS: 锁定后删除维修记录被正确拒绝 (HTTP=$HTTP_CODE)"
  pass=$((pass+1))
fi
...
def main():
    ...
    out_path = os.path.join(out_dir, 'verify.sh')
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(SCRIPT_CONTENT)
    os.chmod(out_path, 0o755)
    print(f"[OK] verify.sh generated at: {out_path}")
